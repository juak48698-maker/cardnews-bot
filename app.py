...
[JSON 형식 유의사항] 모든 문자열 값은 실제 줄바꿈(엔터) 대신 반드시 \n 이스케이프를 써. reels_script_a/b는 자막용으로 줄바꿈이 많이 필요하니 \n을 자유롭게 써도 되지만, 그 외 필드는 되도록 한 줄로 써. 문자열 안에서 인용이 필요하면 큰따옴표(") 대신 작은따옴표(')나 「」를 사용해.

릴스 대본 2가지 (reels_script_a, reels_script_b) — 아래 구조를 반드시 따라서 작성해줘. 목표는 정보 전달이 아니라 시청 유지율(끝까지 보게 만들기)이야.

① 후킹(0~3초): 기존 상식을 깨거나 궁금증을 유발하는 질문/단정으로 시작. 절대 설명으로 시작하지 마
   예) "삼성전자만 사는 거 아니죠?" / "아직도 이렇게 투자하세요?" / "대부분 이걸 모르고 있습니다"
② 결과 먼저 제시(3~6초): 이유 설명 없이 결론부터 짧게 던지기
   예) "앞으로 가장 많이 오를 종목입니다" / "진짜 수혜주는 따로 있습니다"
③ 배경 설명(6~15초, 필요할 때만): 결과가 나온 이유(산업 트렌드/정책/시장상황/기술변화 등 중 자연스러운 것). 억지로 넣지 마, 필요 없으면 생략
...
def process_generation(chat_id, text):
    ...
    try:
        send_message(chat_id, "카드뉴스 만드는 중이에요, 잠시만 기다려주세요... (체크리스트 조사를 위해 웹검색도 하고 있어서 조금 걸릴 수 있어요, 중단하려면 '취소'라고 보내주세요)")
...
def process_package(chat_id, topic, kind="both"):
    ...
    try:
        ...
        send_message(chat_id, f"'{topic['question']}' 주제로 {label} 만드는 중이에요... (체크리스트 조사를 위해 웹검색도 하고 있어서 조금 걸릴 수 있어요)")
