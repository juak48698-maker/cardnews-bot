...
[슬라이드 5] CTA
- "게시글의 추가 내용이 궁금하다면?" 톤
- 정확히 "팔로우 후 댓글에 [이모지] 남겨주시면" 패턴으로 유도해줘 (이모지는 emoji 필드와 통일)
...
- 5~7개 항목, 각 항목은 title(점검 포인트 제목, 짧게) / why(왜 중요한지, 참고자료의 어떤 사실과 연결되는지 1~2문장) / how(구체적으로 무엇을, 어떻게 확인하면 되는지 1~2문장) 세 부분으로 상세하게
...
{{
  ...
  "slides": [
    ...
    {{"emoji": "💬", "title": "슬라이드5 헤드라인", "body": "팔로우 후 댓글에 [이모지] 남겨주시면 유도 문구 (DM/발송 언급 금지)", "search_query": "영어검색어"}}
  ],
...
- slides[3](슬라이드5/CTA): 정확히 "팔로우 후 댓글에 [이모지] 남겨주시면" 패턴으로 유도 (이모지는 emoji 필드와 통일). DM이나 자료 전송 방식은 카드 문구에 절대 언급하지 마
문장은 짧고 단정적으로, 각 슬라이드는 다음 장이 궁금해지는 여운으로 마무리.

[슬라이드4에서 예고한 체크리스트를 실제로 작성해줘 — 발송용 자료]
- 카드에는 절대 내용을 공개하지 않지만, 이 체크리스트는 실제로 발송할 자료라서 진짜로 채워야 해
- {checklist_note}
- 5~7개 항목, 각 항목은 title(점검 포인트 제목, 짧게) / why(왜 중요한지, 근거자료의 어떤 사실과 연결되는지 1~2문장) / how(구체적으로 무엇을 어떻게 확인하면 되는지 1~2문장) 세 부분으로 상세하게
- 특정 종목 매수/매도 지시, 목표가, 수익률 예측 같은 직접적 투자 지시는 절대 쓰지 마 (정보 제공까지만)
...
{{
  ...
  "slides": [
    ...
    {{"emoji": "🙌", "title": "...", "body": "...", "search_query": "영어검색어"}},
    {{"emoji": "💬", "title": "...", "body": "팔로우 후 댓글에 [이모지] 남겨주시면 유도 문구 (DM/발송 언급 금지)", "search_query": "영어검색어"}}
  ],
  "caption": "질문 1줄 + 투표/댓글 유도, 짧게",
  "checklist": [
    {{"title": "점검 포인트 제목", "why": "왜 중요한지 (근거자료 사실 연결)", "how": "구체적으로 어떻게 확인하는지"}}
